  ...
  template: `
    <div class="space-y-6 max-w-3xl mx-auto">
      <div class="flex items-center gap-4 pb-4 border-b border-gray-200">
        ...
        <div>
          <h1 class="text-2xl font-bold text-brand">Designations</h1>
          <p class="text-gray-500 text-sm mt-1">Manage the designation list used across employee records</p>
        </div>
      </div>
...
export class DesignationMaster implements OnInit {
  ...
  load() {
    this.designationService.getAll().subscribe(res => this.designations.set(res));
  }
  ...
  remove(d: Designation) {
    if (!confirm(`Delete designation "${d.name}"?`)) return;
    this.designationService.delete(d.id).subscribe({
      next: () => this.load(),
      error: (err) => this.apiError.set(err.error?.message || 'Failed to delete designation.')
    });
  }
  ...
  onSubmit() {
    ...
    const req = this.editingId()
      ? this.designationService.update(this.editingId()!, data)
    ...
    req.subscribe({
      ...
      error: (err) => this.apiError.set(err.error?.message || 'Failed to save designation.')
    });
  }
}
